...
@if ($available)
    <a href="{{ route($item['route']) }}" @class(['nav-item-link', 'active' => request()->routeIs($item['route']), 'compact' => $compact]) @if(request()->routeIs($item['route'])) aria-current="page" @endif><x-ui.icon :name="$item['icon']" /><span>{{ $item['label'] }}</span>@if(($item['upcoming'] ?? false) && ! $compact)<span class="nav-soon">Presto</span>@endif</a>
@else
